...
class StockData:
    ...
    def data_from_SQL(self, stock_codes: list, dates: list, field: str) -> DataFrame:
        ...
        cursor.execute(sql)
        SQLdata = cursor.fetchall()  # 从本地SQL返回的数据行列表，每行元素分别为stock_code,date,value

        # 将每个value录入DataFrame对应单元格，并从未录入清单中剔除对应code，date和field组合
        for row in SQLdata.copy():
            if (row[-1] == None) or (self.update == True):  # 如果为空值或需要更新数据，不做任何处理
                pass
            else:
                self.SQL_data.loc[row[0], row[1]] = row[-1]  # 录入数据
